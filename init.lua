...
function GetOS()
    ...
    if sep == '/' then return OS_NAME.UNIX
    elseif sep == '\\' then return OS_NAME.WINDOWS
    else return OS_NAME.UNKNOWN end
end
...
vim.api.nvim_create_user_command("Compile", function(opts)
    ...
    if not cmd then
        if is_run_mode and file_exists(os_cmds.run) then cmd = os_cmds.run
        elseif not is_run_mode and file_exists(os_cmds.build) then cmd = os_cmds.build
        else return end
    end
    vim.cmd("below split")
    vim.cmd("term")
...
require('lazy').setup({ 
...

    {
        ...
        config = function()
            ...
            require'nvim-treesitter'.install {"c", "javascript", "typescript", "vue", "bash", "cmake", "yaml",
                    "c_sharp", "cpp", "go", "html", "css", "json", "python",
                    "tsx", "lua", "vim", "vimdoc", "query", "gdscript" } 
...

    {
        ...
        config = function()
            ...
            local function my_on_attach(bufnr)
                ...
                local function opts(desc) return { desc = desc, buffer = bufnr, noremap = true, silent = true } end
            ...
            require("nvim-tree").setup({
                ...
                filesystem_watchers = {
                    ignore_dirs = {
                        "node_modules", ".next", "cmake-*", ".vs", ".idea", ".claude", ".next"
                    }
                },
...

    {
        ...
        config = function()
            ...
            require('mason-tool-installer').setup({
                ensure_installed = {
                    -- Mason package names
                    "omnisharp", "lua-language-server", "python-lsp-server",
                    "typescript-language-server", "vue-language-server",
                    "html-lsp", "css-lsp", "cmake-language-server", "clangd", "gopls",
                    -- Formatters (for conform)
                    "stylua", "goimports", "clang-format", "prettier"
                }
            })
        ...
        vim.api.nvim_create_autocmd('LspAttach', {
            ...
            callback = function(event)
                ...
                vim.keymap.set('n', 'gd', goto_first_definition, { buffer = bufnr, desc = 'LSP: Goto first definition' })
                vim.keymap.set('n', 'gD', vim.lsp.buf.definition, { buffer = bufnr, desc = 'LSP: Goto all definitions' })
                vim.keymap.set('n', 'K', vim.lsp.buf.hover, { buffer = bufnr, desc = 'LSP: Hover' })
                vim.keymap.set('n', '<leader>vd', function() vim.diagnostic.open_float() end, { buffer = bufnr, desc = 'LSP: Open diagnostic float' })
...

    {
        ...
        cmd = { 'ConformInfo' },
        keys = {
            { '<leader>m', function() vim.cmd('GenericFormat') end, desc = '[M]y format' },
        },
        opts = {
